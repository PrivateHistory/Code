...
public class ClhAdvertise {
    ...
    private static final int mUniqueManuID = 1510;
    ...
    private ArrayList<ClhAdvertisedData> waitingList;
    ...
    public void nextAdvertisingPacket() {
        //This can probably be made more optimal
        if (routing.getRoute()!=-1) {//There is a route to the sink
            if(waitingList.size()>0){//Packets waiting for new route
                    ClhAdvertisedData mAdvData = waitingList.get(0);
                    //we set the immediate destination
                    mAdvData.setImmediateDestination((byte)routing.getRoute());
                    byte[] byteData = waitingList.get(0).getParcelClhData();
                    mClhAdvDataList.remove(0);
                    updateCLHdata(byteData);
                    Log.i(LOG_TAG, "new data: size:" + mClhAdvDataList.size() + ",data:" + Arrays.toString(byteData));
            }
            else if(mClhAdvDataList.size()>0) {//packets waiting to be sent
                ClhAdvertisedData mAdvData = mClhAdvDataList.get(0);
                //we set the immediate destination
                mAdvData.setImmediateDestination((byte)routing.getRoute());
                byte[] byteData = waitingList.get(0).getParcelClhData();
                mClhAdvDataList.remove(0);
        ...
        } else if(mClhAdvDataList.size() > 0){//We have pcakets waiting but no route
            ClhAdvertisedData mAdvData = waitingList.get(0);
            mClhAdvDataList.remove(0);
            waitingList.add(mAdvData);
            //If this is the first packet to be sent
            //Or we have waited some time for a response
            if(routing.getSentRREQ()==-1||routing.getSentRREQ()+ RoutingAlgorithm.WAITTIME <date.getTime()){
                //We send the RREQ to the sink
                updateCLHdata(routing.RREQ(mClhID, mClhID, (int)mAdvData.getDestinationID()).getParcelClhData());
                routing.setSentRREQ(date.getTime());
            }else {
                mBleClhAdvertisingStatus = BLE_CLH_ADVERTISING_STATUS_NO_DATA;
                mAdvertisingTimer.start(); // start timer to periodly check (100ms) the list later}
            }
         //TODO I think this also works if we remove all the elses and just put this at the end I dont know and I did not want to break things
        }else{
            mBleClhAdvertisingStatus = BLE_CLH_ADVERTISING_STATUS_NO_DATA;
            mAdvertisingTimer.start(); // start timer to periodly check (100ms) the list later}
            }
    ...
    public void addAdvPacketToBuffer(ClhAdvertisedData data, boolean isOrginal) {
        if (mClhAdvDataList.size() < mMaxAdvAllowable) {
            ...
            } else {//received packet over BLE scan, from other cluster head -> increase hopscount
    ...
    public void addAdvSoundData(byte[] data) {
        ...
        if ((data != null) && data.length > 0) {
            ...
            if (mSoundcount++ == 100) {//wait 100 dataset to reduce the load, update the sound data to advertising list
 ...
 --------*/
    private int startAdvertiser(byte[] settings, byte[] data) {
        ...
        if (data.length < 2) {
            ...
            if (advDatalen > 31) {//if data length too long, send UUID
    ...
    public void setAdvSettings(byte[] settings) {
        ...
        if (settings == null) {   // default advertising: low power, name and UUID , no TX power
